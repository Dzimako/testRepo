...
public class ReportServiceImpl implements ReportsService {
    ...
    private Set<ReportRights> extractRightsFromUser(User user) {
        ...
        Set<ReportRights> reportRightsSetToReturn = new HashSet<ReportRights>();
        reportRightsSetToReturn.addAll(reportRightsSet);
        for (UserGroup userGroup: user.getUserGroupSet()){
            reportRightsSetToReturn.addAll(userGroup.getReportRights());
        }
        return reportRightsSetToReturn;
    }
...
//                System.out.println("отображаем" + dto.getId());
            } else if (cloneNodeTreePath.size() == nodeTreePath.size()){
//                    System.out.println("отображаем" + dto.getId());
            } else {
