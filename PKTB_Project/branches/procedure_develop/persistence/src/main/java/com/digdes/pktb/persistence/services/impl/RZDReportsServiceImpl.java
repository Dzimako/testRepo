...
//        System.out.println("retrieveReport: " + retrieveReport);
        ...
        logger.debug("Retrive report: " + retrieveReport);
    ...
    public GetReportResponse getReportFromDB(GetReportRequest getReportRequest) { //String datasource, String proc_name, List<Pair> parameters, Long reportId,
        ...
        if (getReportRequest.getResultSets() != null) {
            for (ResultSetBean resultSetBean : getReportRequest.getResultSets()) {
                logger.debug("possibleResultset: " + resultSetBean.getKey());
                List<Pair> params = getReportRequest.getPairList();
                if (resultSetBean.getParams() != null && resultSetBean.getParams().size() > 0) {
                    params = getParamsToCertainRequest(resultSetBean.getParams(), getParametersMappedByName(getReportRequest.getPairList()));
                }
                resultSetMap.put(resultSetBean.getKey(), rzdReportDao.callProcedure(resultSetBean.getDatasource(), resultSetBean.getProc_name(), resultSetBean.getSqlStatement(), params));
            }
        }
        ProcedureResultSet procedureResultSet = rzdReportDao.callProcedure(getReportRequest.getDatasource(), getReportRequest.getProc_name(), getReportRequest.getSqlStatement(), getReportRequest.getPairList());
        RetrievedReportXML retrievedReportXML = new RetrievedReportXML();
        if (getReportRequest.getAttributesResultSet() != null) {
            logger.debug("attributes resultSet != null");
        ...
        if (resultSetMap.values() != null && !resultSetMap.values().isEmpty()) {
            logger.debug("resultSetMap is not empty! " + resultSetMap.keySet());

//            logger.debug("Values: " + resultSetMap.values());
            retrievedReportXML.setTables(getTableXMLsFromResult(resultSetMap, getReportRequest));
        } else {
...
//                logger.debug(dictionaryMap);
    ...
    private List<TableXML> getTableXMLsFromResult(ProcedureResultSet procedureResultSet, GetReportRequest getReportRequest) {
        ...
        if (procedureResultSet != null) {
            logger.debug("Num of colls in result set: " + procedureResultSet.getNumOfColumns());
...
//        Calculator calculator = new Calculator(procedureResultSet, mapDictionariesByKey);
        if (resultSetsMap != null) {
            ...
            if (getReportRequest.getTableBeans() != null) {
                logger.debug("There are some tableBeans");
                for (TableBean tableBean : getReportRequest.getTableBeans()) {
                    logger.debug("Want to get procedure from map with a key: " + tableBean.getInputResultSet().getKey());
                    ProcedureResultSet procedureResultSet = resultSetsMap.get(tableBean.getInputResultSet().getKey());
                    if (procedureResultSet != null) {
                        logger.debug("Got resultSet from map! num of cols in it: " + procedureResultSet.getNumOfColumns());
                        Calculator calculator = new Calculator(procedureResultSet, resultSetsMap, mapDictionariesByKey);
                        TableXML tableXML = new TableXML();
                        List<RowXML> rowXMLs = new ArrayList<RowXML>();
                        int numColumns = procedureResultSet.getNumOfColumns();
                        logger.debug("Num of columns: " + numColumns);
                        Integer rowIndex = 1;
                        procedureResultSet.beforeFirst();
                        while (procedureResultSet.next()) {
                            RowXML rowXML = new RowXML();
                            ...
                            if (tableBean.getColumns() != null && tableBean.getColumns().size() > 0) {
                                for (ColumnBean columnBean : tableBean.getColumns()) {
//                                    checkOperatorArguments(columnBean.getOperator(), procedureResultSet.getNumOfColumns());
                                    String valueToAdd = calculator.calculateCellValue(columnBean.getOperator(), rowIndex, null);
                                    if (valueToAdd == null || valueToAdd.equalsIgnoreCase("")) {
                                        valueToAdd = "-";
                                    }
                        ...
                        logger.debug("creating output resultSet. Num of rowXMLs: " + rowXMLs.size());
                        resultSetsMap.put(tableBean.getOutputResultSet().getKey(), createProcedureResultSet(rowXMLs));
                        logger.debug("Num of columns in out resultSet: " + resultSetsMap.get(tableBean.getOutputResultSet().getKey()).getNumOfColumns());


                        if (tableBean.getRowFunctions() != null && tableBean.getRowFunctions().size() > 0) {
                            logger.debug("There are some row functions");
                            for (RowFunctionBean rowFunctionBean : tableBean.getRowFunctions()) {
                                ProcedureResultSet resultSetToUse = procedureResultSet;
                                if (rowFunctionBean.getResultSetToUse() != null) {
                                    logger.debug("Row function. Want to get resultSet from map by key: " + rowFunctionBean.getResultSetToUse().getKey());
                                    if (resultSetsMap.containsKey(rowFunctionBean.getResultSetToUse().getKey())) {
                                        logger.debug("Map contains this key");
                                ...
                                logger.debug("RowFunction: " + rowFunctionBean);
                                rowXMLs = calculatorForRowFunc.calculateRowFunctionRows(rowFunctionBean);
    ...
    /*private void checkOperatorArguments(OperatorBean operatorBean, Integer numOfColsInRow) {
        ...
        if (operatorBean != null && operatorBean.getColumnRuleBean() != null) {
            ...
            if (operatorBean.getColumnRuleBean().getFormulaBean() != null) {
                List<Integer> colNums = getIndexesByRule(operatorBean.getColumnRuleBean().getFormulaBean(), numOfColsInRow);
        ...
        if(operatorBean.getArguments() == null){
            operatorBean.setArguments(new ArrayList<ArgumentBean>());
        }
        operatorBean.getArguments().addAll(argumentBeans);
    }

    private List<Integer> getIndexesByRule(ColumnNumFormulaBean columnNumFormulaBean, Integer numOfColsInRow) {
        ...
        if (columnNumFormulaBean != null && columnNumFormulaBean.getNumToBegin() != null && columnNumFormulaBean.getFormulaString() != null) {
            ...
            Integer endRecursionValue = numOfColsInRow;
