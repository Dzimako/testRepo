...
pktb.init = function (vars) {
    ...
    pktb.getReport = function (data) {
        var requestBean = {reportId:data.reportId, userBean:{userId:currentUser.userId},
            datasource : jQuery("#pktb_project_requestOptions").attr("datasource"),
            proc_name : jQuery("#pktb_project_requestOptions").attr("procedureName"),
            sqlStatement : jQuery("#pktb_project_requestOptions").attr("sqlStatement")}; //datasource    proc_name
...
//        pktb.find('.report .body').load(cr + 'reports/getReport.action', {requestBean:JSON.stringify(requestBean)});
    ...
    pktb.getReportFromDB = function(data){
        var requestBean = {reportId:data.reportId,
            userBean:{userId:currentUser.userId},
            datasource : jQuery("#pktb_project_requestOptions").attr("datasource"),
            proc_name : jQuery("#pktb_project_requestOptions").attr("procedureName"),
            sqlStatement : jQuery("#pktb_project_requestOptions").attr("sqlStatement")}; //datasource    proc_name
        var pairList = new Array();
        ...
        var possibleRequests = new Array();
        if (typeof(data.requestParams) != "undefined") {
            pairList = pktb.getRequestPairsFromString(data.requestParams);
        } else {
            pairList = pktb.getReportRequestPairs();
            ...
            try{
                possibleRequests = pktb.possibleRequests();
                        } catch (e){
                            log('possibleRequests:FAIL');
                            log(e.message);
                        }
        }
        requestBean.pairList = pairList;
        ...
        requestBean.resultSets = possibleRequests;
        jQuery.ajax({
            type:'POST',
            dataType:"html",
    ...
    pktb.possibleRequests = function () {
            log('possibleRequests');
            var possibleRequests;
            var possibleRequestsInput = pktb.find(".possibleRequests").find('input');
        possibleRequests = JSON.parse(jQuery(possibleRequestsInput).val()).resultSets;
...
//                    pairsArray.push({name:jQuery(inputElem).attr("param_date"), value:selectedDate.format("yyyy-mm-dd")});
    ...
    $('#reportsTreeExpandCollapseExpand').click(function(){
        $('#tree').jstree('open_all')
        return false
    })
    $('#reportsTreeExpandCollapseCollapse').click(function(){
        $('#tree').jstree('deselect_all').jstree('close_all')
        return false
    })
