        ...
        <div>
            ...
            <xsl:for-each select="tables/Table">
                <xsl:variable name="i" select="position()"/>
				 <xsl:if test="not(rows/Row[1]/values/string[1]) or rows/Row[1]/values/string[1] != 'HEADER'">
                <xsl:choose>
                <xsl:when test="../../reportOutputTemplate/tables/table[$i]/reportType != ''">
					...
					<xsl:value-of select="reportType"/> -->
                        <xsl:choose>
                            <xsl:when test="reportType = 'headersFormTable'">
                                        ...
                                        <thead class="ui-state-default">
                                            <!--<xsl:choose>
                                                ...
                                                <xsl:otherwise>
                                                    <tr>
                                                        <xsl:for-each
                                                                select="../../../../tables/Table[1]/rows/Row[1]/values/string">
                                                            <th></th>
                                                        </xsl:for-each>
                                                    </tr>
                                                </xsl:otherwise>
                                            </xsl:choose>-->
                                            <!--<xsl:value-of select="../../../tables/Table[values/value = 'HEADER']" />-->
                                            <xsl:apply-templates select="../../../tables/Table[2]">
                                                                                                    </xsl:apply-templates>
                                        </thead>
                                        <tbody>
    ...
    <xsl:template match="Table">
            <xsl:for-each select="rows/Row">
                ...
                <xsl:for-each select="../Row">
                    <xsl:if test="$index = values/string[4]*1">
                    <th>
                        <xsl:if test="values/string[2] != ''">
							<xsl:if test="values/string[2] != '1'">
								<xsl:attribute name="rowspan"> <xsl:value-of select="values/string[2]"/> </xsl:attribute>
							</xsl:if>
                        </xsl:if>
                        <xsl:if test="values/string[3] != ''">
                            <xsl:attribute name="colspan">
                                <xsl:value-of select="values/string[3]"/>
                            </xsl:attribute>
                        </xsl:if>
                        <xsl:value-of select="values/string[1]"/>
                    </th>
                    </xsl:if>
                </xsl:for-each>
                </tr>
