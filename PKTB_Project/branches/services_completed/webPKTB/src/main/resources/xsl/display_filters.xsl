...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="filter">
        <div class="filterItem">
            ...
            <xsl:if test="(type = 'SELECT') or type = ('SELECT_WITH_PARAMS')">
                <select class="input">
                    <xsl:if test="relativeFilterKeys != ''">
                                    <xsl:attribute name="requestParams">
                                                        {"paramFilters" : [
                                                        <xsl:for-each select="relativeFilterKeys/relativeFilterKey">
                                                            {"key" : "<xsl:value-of select="filterKey"/>", "name" : "<xsl:value-of select="paramName"/>"}
                                                            <xsl:if test="following-sibling::*">,</xsl:if>
                                                        </xsl:for-each>
                                                        ]}
                                                    </xsl:attribute>
    ...
    <xsl:template match="filterValue">
        <option>
            ...
            <xsl:attribute name="class">staticOption</xsl:attribute>
            <xsl:value-of select="label"/>
        </option>
    </xsl:template>
    ...
    <xsl:template match="table">
        {
        "inputResultSet" : { "name" : "<xsl:value-of select="inputResultSet/key"/>"},
        "outputResultSet" : { "name" : "<xsl:value-of select="outputResultSet/key"/>"},
        "columns" : [
    ...
    <xsl:template match="rowFunction">
        ...
        "resultSetToUse" : {"name" : "<xsl:value-of select="outputResultSet/key"/>"}
            }
            <xsl:if test="following-sibling::*">,</xsl:if>
        </xsl:template>
