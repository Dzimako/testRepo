...
public class ReportsController extends MultiActionController {
    ...
    public ModelAndView getReport(HttpServletRequest request, HttpServletResponse response) throws Exception {
        Date enterControllerTime = new Date();
        Gson gson = new Gson();
        ...
        GetReportResponse reportContent = rzdReportsService.getReport(getReportRequest.getUserBean(), getReportRequest.getReportId(), getReportRequest.getPairList(), getReportRequest.getProc_name());
        reportContent.setEnterControllerTime(enterControllerTime.getTime()/1000d);


        ModelAndView modelAndView = new ModelAndView("bodyView");
        Date exitControllerTime = new Date();
        reportContent.setExitControllerTime(exitControllerTime.getTime()/1000d);
...
//        requestBean.setUserBean(userBean);
        return new ModelAndView(JsonView.VIEW_JSON, JsonView.JSON_OBJECT, rzdReportsService.getDictionary(requestBean.getUserBean(), requestBean.getFilterUID(), requestBean.getParams()));
    }
...
//        return new ModelAndView(JsonView.VIEW_JSON, JsonView.JSON_OBJECT, reportStatService.getReportStatByReportId(id));
    ...
    public ModelAndView getReportFromDB(HttpServletRequest request, HttpServletResponse response) throws Exception {
        Gson gson = new Gson();
        GetReportRequest getReportRequest = gson.fromJson(request.getParameter("requestBean"), GetReportRequest.class);
        return new ModelAndView(JsonView.VIEW_JSON, JsonView.JSON_OBJECT, rzdReportsService.getReportFromDB(getReportRequest.getDatasource(), getReportRequest.getProc_name(), getReportRequest.getPairList(), getReportRequest.getReportId(), getReportRequest));
    }
    ...
    public ModelAndView createFolder(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String name = request.getParameter("name");
        System.out.println("parentId==================="+request.getParameter("parentId"));
        if (name == null) return null;

        Long parentReportId = request.getParameter("parentId") == null
                ? null
                : Long.parseLong(request.getParameter("parentId"));

        return new ModelAndView(JsonView.VIEW_JSON, JsonView.JSON_OBJECT, reportsService.createFolder(parentReportId, name));
    }
}