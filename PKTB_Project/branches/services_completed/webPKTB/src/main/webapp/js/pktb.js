...
pktb.init = function (vars) {
        ...
        var tree = pktb.find('.tree').jstree({
            ...
            "contextmenu":{
                ...
                "create" : {
                    ...
                    "submenu":{
                        "create_folder":{
                            "separatot_before":false,
                            "separator_after":false,
                            "label":"Каталог",
                            action:function(obj){
                                /* ajax:{
                                 url:cr + '/reports/create.action',
                                 dataType: ,
                                ...
                                $('#create-report-folder').dialog({
                                    ...
                                    buttons:{
                                        "Создать":function(){
                                            $('#create-report-folder-form').submit();
                                        }
                                    }
    ...
    pktb.find(".report .body .innerReportLink").live("click",  function (e) {
                                              pktb.getReport({requestParams:jQuery(e.target).attr("requestparams")});
                                          });
...
//        pktb.find(".innerReportLink").length;
//                          var bodyElem = pktb.find(".report .body .innerReportLink");
        /*pktb.find(".report .body .innerReportLink").bind("click",  function (e) {
                                          pktb.getReport({requestParams:jQuery(e.target).attr("requestparams")});
                                      });*/

                          /*jQuery(pktb.find(".report .body .innerReportLink")).each(function(){
                              jQuery(this).bind("click",  function (e) {
                                  e.data.requestParams = jQuery(e.target).attr("requestparams");
        ...
        jQuery.ajax({
            ...
            success:function (r) {
                if (!r.status) {
                    ...
                    pktb.find('.report .responseReportName').empty();
            ...
            if(jQuery(this).attr("validation") != ""){
                ...
                switch (validationOptions.type){
                    case "NO_TEXT" : var valid = pktb.validateNoTextInputs(jQuery(this).find("input"));
                        ...
                        break;
                    case "NOT_SELECTED" : var validSelect = pktb.validateNotSelectedOption(jQuery(this).find("select"));
                        if (!validSelect) {
                            validated = false;
                            errors.push("Не указан параметр \"" + jQuery(this).find("label").html() + "\"");
                        }
    ...
    pktb.validateNotSelectedOption = function(selectElem){
    ...
    pktb.initFilterRelations = function(){
        ...
        pktb.initRelativeByParamFilters();
    };
    ...
    pktb.initRelativeByParamFilters = function () {
            jQuery(".filterItem select[requestParams]").each(function () {
                var paramsKeys = JSON.parse(jQuery(this).attr("requestParams"));
//                for(var numOfParmeter = 0; numOfParmeter < paramsKeys.length; numOfParmeter++)

//                var paramsKeys = JSON.parse(paramsString);
                if (typeof(paramsKeys.paramFilters) != "undefined") {
                    for (var numOfParam = 0; numOfParam < paramsKeys.paramFilters.length; numOfParam++) {
                        jQuery(".filterItem[paramkey=\"" + paramsKeys.paramFilters[numOfParam].key + "\"] select").bind("change",{selectToReload: jQuery(this).find("select")}, function(event){
                            var selectsArr = new Array();
                            selectsArr.push(event.data.selectToReload.context);
							jQuery(event.data.selectToReload.context).find("option:not(.staticOption)").remove();
                            pktb.getNextDictionary(selectsArr, 0);
                        });
//                        var parameter = pktb.getSinglePairOfFilter(paramsKeys.paramFilters[numOfParam]);
//                        if (parameter != null) {
//                            paramsArr.push(parameter);
//                        }
                    }
                }
//                if (typeof(jQuery(this).attr("relativefilterkey")) != "undefined") {
...
//                }
    ...
    pktb.getNextDictionary = function (selects, numToShow) {
        ...
        var params = pktb.getDictionaryParams(jQuery(selects[numToShow]).attr("requestparams"));
        jQuery.ajax({
            async:false,
            type:'POST',
            dataType:"json",
            ...
            "data":{
                requestBean:JSON.stringify({filterUID:jQuery(selects[numToShow]).attr("serviceuid"), userBean:{userId:currentUser.userId}, params : params})
            },
            success:function (r) {
                if (!r.status) {
    ...
    pktb.getDictionaryParams = function (paramsString) {
        if (typeof(paramsString) != null && paramsString != null && paramsString != '') {
    ...
    pktb.getSinglePairOfFilter = function (relationOptions) {
            if(typeof(relationOptions) != "undefined"){
                return {name:relationOptions.name, value: jQuery(".filterItem[paramkey=\"" + relationOptions.key + "\"] select").val()};
            }
        return null;
        };
    ...
    pktb.getReport = function (data) {
        ...
        if (typeof(data.requestParams) != "undefined") {
            ...
            var reportName = $.grep(pairList, function (item) {
                return (item.name == "_report_name");
            });
            pairList.remove(reportName); //
            requestBean.proc_name = reportName[0].value;
...
//        pktb.find('.report .body').load(cr + 'reports/getReport.action', {requestBean:JSON.stringify(requestBean)});
        ...
        jQuery.ajax({
            ...
            success:function (r, status) {
                r = JSON.parse(r);
                var sendAndRecideTime = (new Date().getTime() - beginTime)/1000;
                var workTimeInConroller = r.exitControllerTime - r.enterControllerTime;
                if (r.success) {
                    jQuery(pktb.find('.report .body')).empty().append(r.response);
                    jQuery(pktb.find('.report .responseReportName')).empty().append(r.reportName);
...
//                pktb.find(".report .body .innerReportLink").length;
                $.unblockUI();
                pktb.find('.clearReportBody').button('option', 'disabled', false);
                var endTime =  new Date().getTime();
                var resultTime = endTime - beginTime;
                resultTime /= 1000;
                jQuery(pktb.find('.report .body')).append("<div>Время выполнения запроса сервисом: "+r.secondsElapsed+"</div>");
                jQuery(pktb.find('.report .body')).append("<div>Время преобразования данных: "+r.estimatedMarshalingTime+"</div>");
//                jQuery(pktb.find('.report .body')).append("<div>Время работы на сервере: "+workTimeInConroller+"</div>");
                jQuery(pktb.find('.report .body')).append("<div>Общее время выполнения от отправки до получения: "+sendAndRecideTime+"</div>");
                jQuery(pktb.find('.report .body')).append("<div>Общее время выполнения метода на клиенте: "+resultTime+"</div>");
            }
        });
